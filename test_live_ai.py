# -*- coding: utf-8 -*-
"""
Script chạy thử nghiệm Master Template Trinh sát Pháp lý (Dữ liệu thật 100%).
Kiểm tra tính năng 1-chạm sao chép số hiệu (<code>), hiển thị quan hệ thay thế,
ngày hiệu lực, quy tắc chuyển tiếp và gửi trực tiếp về Telegram cá nhân.
"""
...
def run_live_test():
    print("🚀 BẮT ĐẦU CHẠY THỬ NGHIỆM MASTER TEMPLATE VỀ TELEGRAM (DỮ LIỆU THẬT 100%)...")
...
Đối với các gói thầu đã phát hành hồ sơ mời thầu trước ngày 27/02/2024 thì tiếp tục đánh giá và ký kết hợp đồng theo Luật Đấu thầu 43/2013 và Nghị định 63/2014. Các gói thầu phát hành từ ngày 27/02/2024 bắt buộc áp dụng Nghị định 24/2024 và các mẫu E-HSMT theo Thông tư 06/2024/TT-BKHĐT.
"""
    ...
    print("🧠 Đang chuyển văn bản cho Gemini AI phân tích theo Master Template...")
    success = process_and_send_alert(item, ai_analyzer, telegraph_pub)

    if success:
        print("✅ THÀNH CÔNG RỰC RỠ! Đã gửi bản tin Master Template hoàn chỉnh kèm Instant View về Telegram của bạn.")
        excel_path = "Kho_Can_Cu_Phap_Ly.xlsx"
        if os.path.exists(excel_path):
            import openpyxl
            wb = openpyxl.load_workbook(excel_path)
            ws = wb.active
            print(f"📊 KIỂM TRA SỔ CÁI EXCEL ({excel_path}):")
            print(f"   • Tổng số dòng: {ws.max_row}")
            print(f"   • Tổng số cột: {ws.max_column} (Chuẩn 14 cột)")
            print(f"   • Dòng mới nhất vừa ghi: {ws.cell(row=ws.max_row, column=4).value} - {ws.cell(row=ws.max_row, column=9).value}")
            wb.close()
    else:
